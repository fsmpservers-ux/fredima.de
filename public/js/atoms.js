...
function drawDeBroglie(a, t) {
  ctx.save(); // ← isoliert den gesamten Zustand
  ...
  orbits.forEach(orb => {
    ...
    ctx.save();
      ctx.globalAlpha = a * .15;  // ← explizit, nicht geerbt
      ctx.strokeStyle = '#ffffff';
      ctx.lineWidth   = 1;
      ctx.setLineDash([3, 7]);
      circ(CX, CY, orb.r);
      ctx.stroke();
    ctx.restore(); // ← lineDash, globalAlpha, alles zurückgesetzt

    // Stehende Welle auf der Kreisbahn
    ctx.beginPath();
    for (let i = 0; i <= 320; i++) {
      const ang  = (i / 320) * Math.PI * 2;
    ...
    // Glow zuerst (dick, transparent)
    ctx.globalAlpha = a * .14;
    ctx.strokeStyle = orb.col;
    ctx.lineWidth   = 6;
    ctx.stroke();

    // Dann scharfe Linie drüber
    ctx.globalAlpha = a * .9;  // ← explizit zurückgesetzt!
    ctx.lineWidth   = 1.8;
    ctx.stroke();
  });

  // Wanderndes Wellenpäckchen auf mittlerer Bahn
  const packetAng = t * .022;
  const packetR   = 88 + Math.sin(packetAng * 4 - t * .036) * 9;
  ...
  ctx.fillText('λ = h/p', CX, CY + 150);

  ctx.restore(); // ← alles sauber zurückgegeben
}
const drawFns = {
